...
def load_data_click(dataset, testing=False):
    """
    Loads data for 247 project
    """
    # x = loadmat('data/douban/training_test_dataset.mat')
    path_dataset = 'data/' + dataset + '/user_item_matrix.npy'

    # M = load_matlab_file(path_dataset, 'M')
    # Otraining = load_matlab_file(path_dataset, 'Otraining')
    # Otest = load_matlab_file(path_dataset, 'Otest')
    M = np.load(path_dataset, allow_pickle=True)
    ...
    if dataset == 'flixster':
        ...
        u_features = np.eye(num_users)
        v_features = Wcol
    elif dataset == 'KDD_247':
        feature_dim = num_users + num_items
        I = np.eye(num_users + num_items)
        u_features = I[0:num_users, :]
        v_features = I[num_users:, :]
    ...
    num_test = np.where(M)[0].shape[0]
    num_val = int(np.ceil(num_train * 0.2))
    num_train = num_train - num_val

    pairs_nonzero_train = np.array([[u, v] for u, v in zip(np.where(M)[0], np.where(M)[1])])
    idx_nonzero_train = np.array([u * num_items + v for u, v in pairs_nonzero_train])

    pairs_nonzero_test = np.array([[u, v] for u, v in zip(np.where(M)[0], np.where(M)[1])])
    idx_nonzero_test = np.array([u * num_items + v for u, v in pairs_nonzero_test])
...
def load_official_trainvaltest_split(dataset, testing=False):
    ...
    u_nodes_ratings, u_dict, num_users, _ = map_data(u_nodes_ratings)
    v_nodes_ratings, v_dict, num_items, _ = map_data(v_nodes_ratings)
    ...
    num_train = data_array_train.shape[0] + data_array_test.shape[0]
...
def load_data_kdd_tmp(dataset, testing=False):
    """
    Loads official train/test split and uses 10% of training samples for validaiton
    For each split computes 1-of-num_classes labels. Also computes training
    adjacency matrix. Assumes flattening happens everywhere in row-major fashion.
    ...
    train_test_split = int(math.ceil(data_array.shape[0] * 0.9))
    ...
    labels = np.full((num_users, num_items), neutral_rating, dtype=np.int32)
    labels[u_nodes, v_nodes] = [rating_dict[r] for r in ratings]
    ...
    if dataset == 'kdd_247':
        print("construct item feature...")
        v_features = np.zeros((num_items, 256), dtype=np.float32)
        #
        # item_data = pd.read_csv('data/' + 'kdd_247' + '/underexpose_item_feat.csv', index_col=False,
        #                         sep='\,\[|\]\,\[|\]', engine='python', names=['item_id', 'txt_vec', 'img_vec'])
        #
        # def check(v_id):
        #     if v_id in v_dict.keys():
        #         return v_dict[v_id]
        #     return False
        # # sorted_by_value = item_data.item_id.apply(lambda v_id: check(v_id)).sort_values(ascending=True)[60678:]
        ...
        # item_data['img_vec'] = item_data['img_vec'].apply(lambda x: map(float,x.split(',')))
        #
        # for item_id, txt_vec, img_vec in zip(item_data['item_id'].values.tolist(), item_data['txt_vec'].values.tolist(), item_data['img_vec'].values.tolist()):
        #     # check if movie_id was listed in ratings file and therefore in mapping dictionary
        #     if item_id in v_dict.keys():
        #         v_features[v_dict[item_id], :128] = txt_vec
        #         v_features[v_dict[item_id], 128:] = img_vec

        # item_data = item_data.values.tolist()

        # drop non-existing items (not clicked before)
        # item_data = np.array([x for x in item_data if int(x[0]) in v_dict.keys()])
        #
        # item_id_vec = list(map(int, item_data[:, 0]))
        # item_idx = list(map(v_dict.get, item_id_vec))
        # v_features[item_idx] = item_data[:, 2:130]
        # np.save('data/' + dataset + "/item_feature_np_{}.npy".format(phase), v_features)
        print("finish")
        v_features = np.load('data/' + dataset + '/item_feature_np_{}.npy'.format(phase), allow_pickle=True)
        v_features.astype(np.float32)
        # np.save('data/' + dataset + "/item_feature_np_{}.npy".format(phase), v_features)
        # user features

        # u_features = np.zeros((num_users, 3), dtype=np.float32)
        u_features = np.load('data/' + dataset + '/user_feature_np_{}.npy'.format(phase), allow_pickle=True)
        u_features.astype(np.float32)

        # one-hot vec for categorical feature
        print("construct user feature...")
        # user_data = pd.read_csv('data/' + dataset + '/underexpose_user_feat.csv', sep=',', names=["user_id", "age", "gender", "city"])
        #
        # user_data = user_data.dropna()
        # gender_dict = {'M': 0, 'F': 1}
        #
        # city = set(user_data['city'].values.tolist())
        # city_dict = {v: k for k, v in enumerate(city, start=2)}
        #
        # age = user_data['age'].values
        # age_max = age.max()
        #
        # num_feats = 2 + len(city_dict)
        #
        # u_features = np.zeros((num_users, num_feats), dtype=np.float32)
        ...
        #         u_features[u_dict[u_id], 1] = gender_dict[row['gender']]
        #         # city
        #         u_features[u_dict[u_id], city_dict[row['city']]] = 1.
        # u_features = np.load('data/' + dataset + "/user_feature_np.npy".format(phase), allow_pickle=True)
        # np.save('data/' + dataset + "/user_feature_np_{}.npy".format(phase), u_features)
        print("finish")
...
def load_data_kdd(dataset, testing=False):
    """
    Loads official train/test split and uses 10% of training samples for validaiton
    For each split computes 1-of-num_classes labels. Also computes training
    adjacency matrix. Assumes flattening happens everywhere in row-major fashion.
    ...
    data_all['ratings'] = data_all['ratings'] * data_all['timestamp']
    data_all = data_all.groupby(['u_nodes', 'v_nodes'])['ratings'].sum().reset_index()
    data_all['ratings'] = np.ceil(data_all['ratings'] / 5) + 1
    data_all = data_all.sort_values(by=['u_nodes']).reset_index(drop=True)
    ...
    u_nodes_ratings, u_dict, num_users, u_dict_rev = map_data(u_nodes_ratings)
    v_nodes_ratings, v_dict, num_items, v_dict_rev = map_data(v_nodes_ratings)
    ...
    np.save('data/' + dataset + "/u_dict", u_dict)
    np.save('data/' + dataset + "/v_dict", v_dict)
    np.save('data/' + dataset + "/u_dict_rev", u_dict_rev)
    np.save('data/' + dataset + "/v_dict_rev", v_dict_rev)
    # return u_features, v_features, rating_mx_train, train_labels, u_train_idx, v_train_idx, \
    #     val_labels, u_val_idx, v_val_idx, test_labels, u_test_idx, v_test_idx, class_values
    return u_features, v_features, rating_mx_train, train_labels, u_train_idx, v_train_idx, \
           val_labels, u_val_idx, v_val_idx, class_values