...
from preprocessing import create_trainvaltest_split, \
    ...
    load_data_monti, load_official_trainvaltest_split, normalize_features, load_data_kdd
...
ap.add_argument("-d", "--dataset", type=str, default="ml_1m",
                choices=['ml_100k', 'ml_1m', 'ml_10m', 'douban', 'yahoo_music', 'flixster', 'kdd_247'
                                                                                            ''],
                help="Dataset string.")
...
if DATASET == 'kdd_247':
    NUMCLASSES = 11    # TO BE UPDATED
elif DATASET == 'ml_1m' or DATASET == 'ml_100k' or DATASET == 'douban':
    NUMCLASSES = 5
elif DATASET == 'ml_10m':
    NUMCLASSES = 10
...
if DATASET == 'kdd_247':
    u_features, v_features, adj_train, train_labels, train_u_indices, train_v_indices, \
        val_labels, val_u_indices, val_v_indices, class_values = load_data_kdd(DATASET, TESTING)

elif DATASET == 'flixster' or DATASET == 'douban' or DATASET == 'yahoo_music':
    u_features, v_features, adj_train, train_labels, train_u_indices, train_v_indices, \
    ...
    u_features, v_features, adj_train, train_labels, train_u_indices, train_v_indices, \
        ...
        class_values = load_official_trainvaltest_split(DATASET, TESTING)
else:
...
if FEATURES:
    # test_u_features_side = u_features_side[np.array(test_u) - 1]
...
for epoch in range(NB_EPOCH):
    ...
    # output_test = sess.run([model.outputs], feed_dict=test_feed_dict)
...
def score(x):
    ...
    for i in range(len(x)):
        s += (i + 1) * x[i]
    return s
output_train = sess.run([model.outputs], feed_dict=train_feed_dict)[0]
output_train = np.array([x / sum(x) for x in output_train])
output_score = np.array([score(x) for x in output_train])

res_matrix = np.zeros((num_users, num_items), dtype=np.float32)
res_matrix[train_u_indices, train_v_indices] = output_score

res_idx = np.zeros((num_users, 50))
for i in range(res_matrix.shape[0]):
    res_idx[i] = res_matrix[i].argsort()[-50:][::-1]

np.save('data/' + DATASET + "/res_matrix_{}.npy".format(NUMCLASSES), res_matrix)

# plt.hist(output_train_max)
# plt.show()

# output_val = sess.run([model.outputs], feed_dict=val_feed_dict)
# output_val_max = np.argmax(output_val[0], axis=1)
# res_matrix[val_u_indices, val_v_indices] = output_val_max
